...
from poetry_lm.rhyme import (
    detect_eight_line_aabb_aabb,
    detect_eight_line_aabb_aabb_from_tails,
...
from poetry_lm.tokenizer import STRUCTURED_SCHEME, structured_window_to_training_text
...
def build_window_record(row: dict, lines: list[str], start_idx: int, confidence: float) -> dict:
    ...
    return {
        ...
        "topic": row.get("topic"),
        "text": text,
        "formatted_text": structured_window_to_training_text(lines),
        "prompt": lines[0],
        "line_count": 8,
        "word_count": count_words(text),
        "rhyme_scheme": STRUCTURED_SCHEME,
...
def flush_pending(split: str, pending: list[tuple[dict, list[str], int]], out_fh, stats: Counter, seen: set[str]) -> None:
    ...
    for batch_idx, (row, window, start_idx) in enumerate(pending):
        ...
        stressed = detect_eight_line_aabb_aabb_from_tails(tails)
        if stressed.scheme != STRUCTURED_SCHEME:
...
def main() -> None:
    ...
    parser.add_argument("--out-dir", default="data/processed_aabb8")
            ...
            for line in tqdm(iterator, desc=f"build_{split}"):
                ...
                for start_idx in window_starts:
                    ...
                    if not poem_is_good(window):
                        ...
                        continue
                    heuristic = detect_eight_line_aabb_aabb(window, use_stress=False)
                    if heuristic.scheme != STRUCTURED_SCHEME:
