...
def main() -> None:
    ...
    parser.add_argument("--base-model", default="ai-sage/GigaChat3-10B-A1.8B-base")
    ...
    base_model = prepare_gigachat_local_model_dir(args.base_model)
    ...
    with gzip.open(args.test_file, "rt", encoding="utf-8") as fh:
        for line in tqdm(fh, total=args.limit, desc="evaluate_gigachat_8line"):
            if total >= args.limit:
                break
            row = json.loads(line)
            prompt = row["prompt"]
