...
For GitHub Actions, save the completed configuration as the encrypted repository secret `DEPLOY_CONFIG`. The workflow writes it to a temporary file outside the checkout with mode 600 and deletes it when the step exits.

Copy `deploy/model-release.env.example` to `/etc/poetry-lm/model-release.env` on your deployment host, outside the checkout. Fill in your own host, account, paths, and ports; restrict the file to the deployment account (`chmod 600`). To use a different private location, set `DEPLOY_CONFIG_FILE`. The deploy script checks that this file is readable before contacting any remote host. Do not commit the real file, operational journals, SSH targets, or local tool settings.
...
Если это по-прежнему `/home/app/projects/Poetry`, используй именно его как `PROJECT_ROOT`.
...
- `PROJECT_ROOT=/home/app/projects/Poetry`
...
```bash
ssh -p 22 app@<GPU_HOST>
```

Если в вашей схеме GPU-хост достигается как `gpu.example.com:22`, настрой именно это. Главное, чтобы tunnel-сервис мог достучаться до GPU-хоста без интерактивного ввода пароля.
